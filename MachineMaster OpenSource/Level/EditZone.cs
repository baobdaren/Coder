...
public class EditZone : SerializedMonoBehaviour
{
	...
	// ----------------//
	[SerializeField]
	private TargetTrigger PalyerTrigger;
	[SerializeField]
	...
	{
		get => PalyerTrigger.TargetStayIn;
	}
	...
	private void Awake()
	{
		Debug.Assert(PalyerTrigger != null, this.name);
		//Debug.Assert(AllSceneParts != null);
		OtherColliders = new List<Collider2D>();

		PalyerTrigger.OnPlayerEnter.AddListener(OpenDesignUI);
		PalyerTrigger.OnPlayerExit.AddListener(CloseDesignUI);
	...
	public bool HasOverBoundPoints(out Vector2[] result)
	{
		...
		{
			castAmount = Physics2D.LinecastNonAlloc(EditArea.points[i - 1], EditArea.points[i - 1], castResult, ~GameObjectLayerManager.EditPart);
			if (castAmount == 0)
			{
				continue;
			}
	...
	public bool HasOverBoundPoints()
	{
		...
		for (int i = 1; i < EditArea.points.Length; i++)
		{
			if (Physics2D.LinecastNonAlloc(EditArea.points[i - 1], EditArea.points[i - 1], castResult, ~GameObjectLayerManager.EditPart) != 0)
			{
				return true;
			}
		}
	...
	private bool IsScenePartCollider(Collider2D collider2D)
	{
		//foreach (var item in AllSceneParts)
		//{
		//	if (item.)
		//	{
		...
		//	}
		//}
		throw new System.Exception("为实现");
	}
