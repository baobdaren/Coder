...
public class ControllerModeling
{
    ...
    public void DisplayModelMap(PlayerPartCtrl partCtrl)
    {
        ...
        foreach (Node nodeItem in Model.UsingPart.ModelMap.AllNodes)
		{
    ...
    /// <returns></returns>
    public Symbol CreateNodeSymbol(Node node)
    {
    ...
    public void CreateNewConnection(Node output, int outputIndex, Node input, int inputIndex)
    {
    ...
    public void SwitchSelectSensor ( bool next)
    {
		...
		{
			UIManager.RenderTexCamera.transform.DOMove(Vector3.back * 10 + Model.CurPreviewSensor.transform.position, 0.6f);
		}
	}
