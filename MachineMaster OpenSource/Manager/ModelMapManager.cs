...
public class ModelMapManager : Loadable<ModelMapManager>
{
	...
	{
		foreach (KeyValuePair<int, (List<Node>, List<ArchiveManager.SymbolConnectArchiveData>)> item in archive.partModelingMapsArchive)
		{
			// 该建模图所属的零件
			PlayerPartCtrl part = PlayerPartManager.Instance.AllPlayerPartCtrls[item.Key];
			// 该建模图所有节点
			List<Node> nodes = item.Value.Item1;
			// 该建模图所有节点连接关系
	...
	{
		archive.partModelingMapsArchive = new Dictionary<int, (List<Node>, List<ArchiveManager.SymbolConnectArchiveData>)>();
